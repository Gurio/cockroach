...
func randAStringInitialDataOnly(rng *tpccRand, a *bufalloc.ByteAllocator, min, max int) []byte {
...
// For speed, this is done using precomputed random data, which is explicitly
// allowed by the spec for initial data only. See 4.3.2.1.
func randNStringInitialDataOnly(rng *tpccRand, a *bufalloc.ByteAllocator, min, max int) []byte {
...
// For speed, this is done using precomputed random data, which is explicitly
// allowed by the spec for initial data only. See 4.3.2.1.
func randStateInitialDataOnly(rng *tpccRand, a *bufalloc.ByteAllocator) []byte {
...
// For speed, this is done using precomputed random data, which is explicitly
// allowed by the spec for initial data only. See 4.3.2.1.
func randOriginalStringInitialDataOnly(rng *tpccRand, a *bufalloc.ByteAllocator) []byte {
	if rng.Rand.Intn(9) == 0 {
		l := int(randInt(rng.Rand, 26, 50))
		off := int(randInt(rng.Rand, 0, l-8))
		var buf []byte
		*a, buf = a.Alloc(l, 0 /* extraCap */)
		copy(buf[:off], randAStringInitialDataOnly(rng, a, off, off))
		copy(buf[off:off+8], originalString)
		copy(buf[off+8:], randAStringInitialDataOnly(rng, a, l-off-8, l-off-8))
		return buf
	}
	return randAStringInitialDataOnly(rng, a, 26, 50)
}
...
// For speed, this is done using precomputed random data, which is explicitly
// allowed by the spec for initial data only. See 4.3.2.1.
func randZipInitialDataOnly(rng *tpccRand, a *bufalloc.ByteAllocator) []byte {
	var buf []byte
	*a, buf = a.Alloc(9, 0 /* extraCap */)
	copy(buf[:4], randNStringInitialDataOnly(rng, a, 4, 4))
	copy(buf[4:], `11111`)
	return buf
}
